...
class ChronologyRepository:
    ...
    def _rows_to_entries(rows: Iterable[sqlite3.Row]) -> List[HistoryEntry]:
        ...
        for row in rows:
            ...
            out.append(HistoryEntry(
                year_ad=row["公元"], ganzhi=row["干支"], period=period, regime=regime,
                emperor_title=row["帝号"], emperor_name=row["帝名"],
                reign_title=row["年号"], regnal_year=row["年份"],
            ))
        return out
    ...
    def _generate_variants(self, text: str) -> Set[str]:
        ...
        variants.add(zhconv.convert(text, 'zh-hant'))
        variants.add(zhconv.convert(text, 'zh-hans'))
        return variants
    ...
    def get_entries_by_year(self, year: int) -> List[HistoryEntry]:
        cur = self._conn.execute("SELECT * FROM history_chronology WHERE 公元 = ? ORDER BY 年份", (year,))
    ...
    def advanced_query(self, *, year_from: Optional[int] = None, year_to: Optional[int] = None,
                       ganzhi: str | None = None, period: str | None = None, regime: str | None = None,
                       emperor_title: str | None = None, emperor_name: str | None = None,
                       reign_title: str | None = None) -> List[HistoryEntry]:
        conditions, params = [], []
        ...
        if ganzhi: add_text_condition("干支", ganzhi)
        if period: add_text_condition("时期", period)
        if regime: add_text_condition("政权", regime)
        if emperor_title: add_text_condition("帝号", emperor_title)
        if emperor_name: add_text_condition("帝名", emperor_name)
        if reign_title: add_text_condition("年号", reign_title)
